  ...
  async function loadNews(e: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        `https://api.spaceflightnewsapi.net/v4/articles/?format=json&limit=10&search=${query}`
      );
      if (!response.ok) throw new Error(`Ошибка сервера: ${response.status}`);
      const json = await response.json();
      if (!json.results || json.results.length === 0)
        setError(
          "Не удалось получить новости об этом объекте. Попробуйте ввести другое название."
        );
    ...
    <div className="page-container news-container">
      ...
      <form onSubmit={loadNews} className="new-form">
        <input
          ...
          placeholder="Например: Mars, Sun, Pluto"
        />
              ...
              <details className="in-detail">
                ...
                <ul className="meta-data">
                  <li>
                    ...
                    {a.authors?.[0]?.name || "Автор неизвестен"}
                  </li>
                  <li>
                  ...
                  <li>
                    <strong>Источник:</strong> {a.news_site}
                  </li>
                  <li>
